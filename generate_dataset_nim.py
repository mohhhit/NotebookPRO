...
OUTPUT_FILENAME = "qwen_golden_dataset.jsonl"
TEACHER_MODEL = os.getenv("NVIDIA_MODEL", "meta/llama-4-maverick-17b-128e-instruct")
NVIDIA_API_BASE_URL = os.getenv("NVIDIA_API_BASE_URL", "https://integrate.api.nvidia.com/v1")
# Optional direct key paste for Colab/quick experiments.
# Leave empty to use NVIDIA_API_KEY/GEMINI_API_KEY from .env or environment variables.
NVIDIA_API_KEY_INLINE = ""
TARGET_SAMPLES = 500
BACKEND_USE_CASE = "qa"
...
def load_api_key_from_env_file() -> None:
    ...
    for raw_line in env_path.read_text(encoding="utf-8").splitlines():
        ...
        if key == "NVIDIA_API_KEY" and value:
            # Always prefer .env to avoid stale key values in long-lived shells.
            os.environ["NVIDIA_API_KEY"] = value
        elif key == "GEMINI_API_KEY" and value and not os.getenv("NVIDIA_API_KEY"):
            # Fallback for users migrating old env files.
            os.environ["NVIDIA_API_KEY"] = value


def build_nvidia_client() -> Any:
    try:
        ...
        raise ImportError(
            "openai package is required for NVIDIA NIM. Install with: pip install openai"
        ) from exc

    inline_key = NVIDIA_API_KEY_INLINE.strip()
    key_source = "inline variable"
    ...
    if not api_key:
        raise EnvironmentError(
            "NVIDIA API key is not set. Paste NVIDIA_API_KEY_INLINE in the script or set NVIDIA_API_KEY."
        )

    print(f"Using key source: {key_source}")
    print(f"Using NVIDIA_API_KEY fingerprint: {mask_api_key(api_key)}")
    print(f"Using NVIDIA model: {TEACHER_MODEL}")
...
def sanitize_model_json_text(text: str) -> str:
    ...
    # Remove BOM and null bytes sometimes emitted by gateways.
    ...
    for candidate in candidates:
        for attempt_fix in range(2):
            try:
                ...
                if attempt_fix == 0:
                    # Fix invalid backslashes (common with LaTeX) and try once more.
    ...
    if isinstance(content, list):
        ...
        for part in content:
            ...
            if isinstance(part, dict):
                raw = part.get("text") or part.get("content") or part.get("value")
...
def create_chat_completion(
    ...
    try:
        ...
        msg = str(exc).lower()
        if "extra_body" in msg or "chat_template_kwargs" in msg or "thinking" in msg:
            return client.chat.completions.create(**kwargs)
        raise


def repair_json_payload(client: Any, malformed_text: str) -> dict[str, Any] | None:
    repair_prompt = f"""
...
""".strip()
    ...
    try:
        ...
        repaired_text = extract_response_text(repair_response)
...
def clean_teacher_answer(answer: str) -> str:
    ...
    # Remove common meta prefixes leaked by unstable generations.
...
def load_existing_dataset_state(output_path: Path) -> tuple[set[str], int]:
    ...
    for raw_line in output_path.read_text(encoding="utf-8").splitlines():
        ...
        try:
            ...
            # Count slot progress even for malformed historical lines.
            continue
...
def generate_triplet(
    ...
    for attempt in range(max_transient_retries + 1):
        try:
            ...
            if is_transient:
                print("Warning: transient model error (503/timeout). Sleeping for 10 seconds...")
                time.sleep(10)
                if attempt < max_transient_retries:
    ...
    with output_path.open("a", encoding="utf-8") as f:
        for slot_index in range(processed_slots, total_samples):
            ...
            for regen_attempt in range(duplicate_regen_limit + 1):
                ...
                # Respect API rate limits between successful calls.
                time.sleep(1)
                break
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a Universal Expert Tutor dataset using NVIDIA NIM Llama 4 Maverick."
    )
    parser.add_argument(
        "--output-dir",
        default=OUTPUT_DIR,
    ...
    # Jupyter/Colab inject extra argv flags (for example: -f kernel-*.json).
    # parse_known_args keeps script execution robust in notebook runtimes.
